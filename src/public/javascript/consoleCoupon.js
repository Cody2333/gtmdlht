...
.factory('page', function() {
    return {pageName: 'MemberCode', user: {}, userOrder: [], passengers: [], agent: {}};
})
...
.service('getBasicInfo', function($http, tool) {
	...
	this.getDiscountCodes = function(array) {
		array.length = 0;
		$http({url: '/admin/coupon/discountCodeList', method: 'POST', data: {skip: 0, limit: 20}})
		.success(function(res) {
			if (res.status == 'success') {
				// console.log(res.data);
				for (var i = 0; i < res.data.length; i++) {
...
.controller('MemberCode', ['$scope', '$http', 'page', 'getBasicInfo', function MemberCode($scope, $http, page, getBasicInfo) {
	$scope.page = page;
	$scope.iTime = {time0: false, time1: false, time2: false};
	$scope.pageType = 0;
	$scope.discountCodes = [];
	$scope.discountCode = {};
	...
	$scope.add = function() {
		...
		$http({url: '/admin/coupon/addDiscountCode', method: 'POST', data: {name: $scope.discountCode.name, show: $scope.discountCode.show, code: $scope.discountCode.code,
			validDate: getTime($scope.discountCode.validDate), couponValidPrice: $scope.discountCode.couponValidPrice, couponValidSrcId: $scope.discountCode.couponValidSrc.id, couponValidStartTime: getTime($scope.discountCode.couponValidStartTime),
			couponValidEndTime: getTime($scope.discountCode.couponValidEndTime), discountRuleId: $scope.discountCode.discountRule.id}})
		.success(function(res) {
			if (res.status == 'success') {
				alert('添加成功');
			}
	...
	$scope.saveDiscountCode = function(index) {
		...
		$http({url:'/admin/coupon/updateDiscountCode', method: 'POST', data: {discountCodeId: $scope.discountCodes[index].id, show: $scope.discountCodes[index].show,
			validDate: getTime($scope.discountCodes[index].validDate), discountRuleId: $scope.discountCodes[index].discountRule.id, 
...
.controller('DiscountRule', ['$scope', '$http', 'page', 'getBasicInfo', 'tool', function DiscountRule($scope, $http, page, getBasicInfo, tool) {
	...
	$scope.add = function() {
		...
		$http({url: '/admin/coupon/addDiscountRule', method: 'POST',
			data: {description: $scope.discountRule.description, discountFunction: tool.switchToRealFunction($scope.discountRule.discountFunction)}})
		.success(function(res) {
			if (res.status == 'success') {
				alert('添加成功');
			}
...
.controller('UserDiscountCode', ['$scope', '$http', 'page', 'getBasicInfo', function UserDiscountCode($scope, $http, page, getBasicInfo) {
	...
	$scope.save =function() {
		...
		$http({url: '/admin/coupon/updateDiscountRuleMap', method: 'POST', data: {discountRuleMapId: $scope.userDiscountRuleMap.id, show: $scope.userDiscountRuleMap.show,
			validPrice: validPrice, validTime: validTime, discountRuleId: $scope.userDiscountRuleMap.discountRule.id}})
		.success(function(res) {
			if (res.status == 'success') {
				alert('保存成功');
			}
	...
	function getPageInfos() {
		$http({url: '/admin/coupon/userDiscountRuleMap', method: 'POST'})
		.success(function(res) {
			if (res.status == 'success') {
...
.controller('DistributeCoupon', ['$scope', '$http', 'page', 'getBasicInfo', 'tool', function DistributeCoupon($scope, $http, page, getBasicInfo, tool) {
	...
	$scope.add = function() {
		...
		$http({url: '/admin/coupon/distribute', method: 'POST', data: {show: $scope.coupon.show, discountRuleId: $scope.coupon.discountRule.id,
			validSrcId: $scope.coupon.validSrc.id, validDestId: $scope.coupon.validDest.id, validPrice: $scope.coupon.validPrice * 100,
...
.controller('DonateCoupon', ['$scope', '$http', 'page', 'getBasicInfo', 'tool', function DonateCoupon($scope, $http, page, getBasicInfo, tool) {
	...
	$scope.search = function() {
		$http({url: '/admin/user/getByPhone', method: 'POST', data: {phone: $scope.user.phone}})
		.success(function(res) {
			if (res.status == 'success') {
				if (res.data == null) {
					$scope.err = 1;
	...
	$scope.donateCoupon = function() {
		...
		$http({url: '/admin/coupon/donate', method: 'POST', data: {users: $scope.users, show: $scope.coupon.show, discountRuleId: $scope.coupon.discountRule.id}})
		.success(function(res) {
			if (res.status == 'success') {
				alert('赠送优惠券成功');
			}
